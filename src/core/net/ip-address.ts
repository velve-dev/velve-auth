...
function networkBytes(bytes: readonly number[], prefixLength: number): number[] {
	return bytes.map((byte, index) => {
		const keptBits = Math.min(Math.max(prefixLength - index * BITS_PER_BYTE, 0), BITS_PER_BYTE);
		return byte & (0xff << (BITS_PER_BYTE - keptBits));
	});
}

export interface IpAddressPrefixLengths {
...
export function ipAddressNetwork(
	text: string,
	prefixLengths: IpAddressPrefixLengths,
): string | null {
	const address = parseIpAddress(text);
	if (address === null) {
		return null;
	}
	const prefixLength = address.isIpv4 ? prefixLengths.ipv4 : prefixLengths.ipv6;
	const network = networkBytes(address.bytes, prefixLength);
	const networkText = address.isIpv4 ? ipv4Text(network) : ipv6Text(network);
	return `${networkText}/${prefixLength}`;
}
