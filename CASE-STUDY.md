...
**Reason.** They are worth naming because several are the failure class `CLAUDE.md` §6 warns about for decision citations — a reference that does not dangle but resolves to the wrong thing. Section 1's summary line for group A reads `26 · 11 · 12 · 3` against rows that count `23 · 14 · 12 · 3`. `I.2` is headed `(44)` and has 54 rows. 3.15 B.4 says "all three writing methods" of a namespace that has four. The silent Argon2id rehash is cited once as "section 3.3, step 5" where step 6 is meant, against nine correct citations of step 6. 4.1 c) and 4.4 c) map columns their own source-schema tables do not list. T-RATE-1 enumerates twelve vectors and sets a threshold of `20/20`; T-RATE-3 describes three constellations and sets `6/6`; T-REDIR-2 fixes `120/120` over a corpus its own procedure says grows with every finding. 5.7 calls a `/64` prefix "sixty-two-digit numbers of buckets" two lines above the paragraph that states 2^64. 5.10 and 5.13 each claim a different error class is the most productive. 5.19 says a structurally preventing requirement is "named in parentheses" in a table that uses no parentheses for it. Two markdown faults — an unclosed inline-code backtick at 3774 and nested bold at 3049 — render wrongly in the German today.
