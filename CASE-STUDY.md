...
**Context.** `E-889` argues that the version stays at 1 because raising it per commit would publish numbers no reader ever saw, and offers as its evidence that this branch alone would have shipped as version 4. Six commits touch a skill file. `git log origin/main..HEAD -- CLAUDE-SKILL.md CODEX-SKILL.md` lists six; the number was written when four of them existed and was not revisited when the last two landed.
