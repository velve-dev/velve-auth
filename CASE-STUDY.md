...
**Price.** A reader who takes `176 0` as "this branch appended only" reads it wrongly, and this entry is the only thing that says so. Nothing in the repository runs the check yet; adopting it means a gate step, and a gate step means deciding whether the two-dot form — which would have flagged all three edits, including the one E-536 licensed — is the one to run. That decision is not taken here.
