...
**Price.** The difference that decided it cannot be observed anywhere in the current tree — no marker in this repository contains an asterisk — so the resolution rests on a planted input and on nothing else, and it is only worth what that input is worth. The planted case is therefore now a test case in the same file, next to the two faults it must keep rejecting. The collision itself is not repaired by any of this: the file is still shared, nothing stopped either writer from opening it, and the next pair will meet in it the same way. Two crossings happened in this one merge, not one — converting these entries to the English format also forced a change to `test/decision-log.test.ts`, which belongs to no feature either. Both were reported rather than quietly taken; that is the whole of the safeguard, and it is a habit, not a mechanism.
