...
*Preis:* Eine bewusste Abweichung vom Wortlaut der Vorgabe — dieselbe Funktion, anders angewandt. Und die Übereinstimmung endet an der Unicode-Version: Ein Abgleich über alle Codepunkte unter U+30000 findet genau eine Abweichung, U+038D, einen unbelegten Platz, den die C-Bibliothek faltet und JavaScript zu Recht in Ruhe lässt.
...
*Preis:* Eine Funktion, deren Name mit `assert` beginnt, schreibt. Umbenennen ging nicht, ohne die Tests des Prüfers zu brechen, und der Name bleibt damit schwächer als das, was der Aufruf tut. Dazu ein zusätzlicher Umlauf zur Datenbank für die Frage, ob die Sperre hält.
...
**E-207 — `RecoveryCodesRequirement` gehört zu `createVelveAuth` und wird hier nicht gebaut.**
*Kontext:* 3.4 und 3.15 A.3 verlangen, dass `identity: { mode: "username" }` ohne `recoveryCodes` ein Startfehler ist und über `RecoveryCodesRequirement<M>` schon ein Kompilierfehler. Der Typ steht im selben Block wie `IdentityConfig` und existiert nirgends.
...
*Preis:* Bis dahin ist die Konfiguration `username` ohne Wiederherstellungscodes ein Fehler, den die Bibliothek nicht abfängt — genau die Lücke, die E-18 schließen sollte. Die Referenz sagt es an der Stelle, an der jemand die Konfiguration wählt.
