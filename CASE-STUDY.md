...
**E-220 — Gehasht wird der Cookie-Text, nicht die 32 Rohbytes.**
*Kontext:* 3.5 schreibt „gespeichert wird nur `sha256(token)`". Das Token ist 32 Byte Zufall, ausgeliefert als base64url. `sha256(token)` lässt beides zu: den Hash über die Rohbytes oder über den Text, der im Cookie steht.
*Verworfen:* Den Hash über die dekodierten Rohbytes zu bilden.
*Grund:* Dann müsste die Auflösung jeden ankommenden Cookie-Wert zuerst dekodieren, und ein nicht dekodierbarer Wert wäre ein zweiter Fehlerzustand neben „nicht gefunden". Genau daraus entsteht ein Orakel: Wer ein ungültiges Zeichen sendet, bekäme eine andere Antwort als wer ein gültiges, aber unbekanntes Token sendet. Über den Text gehasht gibt es nur einen Zustand — der Hash trifft eine Zeile oder er trifft keine. S-FIX-3 rechnet ohnehin mit `sha256($alt)` über das Token, wie es gesendet wurde.
*Preis:* Es gibt keine Kanonisierung. Dieselben 32 Byte, anders geschrieben — mit Auffüllzeichen oder in Standard-base64 — ergeben einen anderen Hash und damit keine Sitzung. Das ist richtig, aber es überrascht jeden, der das Token unterwegs umkodiert.

**E-221 — Der base64url-Kodierer entsteht in `session/`, obwohl der Dekodierer in `keys/` steht.**
*Kontext:* `src/core/keys/base64url.ts` hat einen Dekodierer und ausdrücklich keinen Kodierer, weil Welle 1 keinen brauchte. Das Sitzungstoken braucht einen, und `btoa` steht nicht in den Laufzeitannahmen aus 2.6.
...
**E-227 — Zeitstempel dekodiert der Treiber, das Repository liest sie nur.**
*Kontext:* `velve.session` hat vier `timestamptz`-Spalten, und `Session` verlangt `Date`. Die naheliegende Fassung war ein `toDate()` im Repository, das sowohl ein `Date` als auch die Textform annimmt, die PostgreSQL über die Leitung schickt.
*Verworfen:* Die Textform im Kern parsen.
*Grund:* Zwei Gründe. Erstens ist die Umwandlung eines PostgreSQL-Typs in einen JavaScript-Wert die Aufgabe des Treibers — genau dafür ist der Treiber ein Parameter (2.6); `node-postgres`, `postgres.js` und der Neon-Treiber liefern alle drei ein `Date`. Zweitens verbietet `test/keys-static-scan.test.ts` `new Date(` in `src/core` überhaupt, damit kein Geheimnis aus einer Uhr entsteht. Ein Parser im Kern hätte diese Prüfung gebrochen oder eine Ausnahme von ihr gebraucht — für eine Aufgabe, die eine Schicht tiefer hingehört.
*Preis:* Der `Driver`-Vertrag verlangt jetzt etwas, das er nicht ausspricht: `query` muss `timestamptz` als `Date` zurückgeben. Es steht in der Dokumentation, nicht im Typ. Und die minimale Testverbindung `test/db-postgres-connection.ts` — eine fremde Datei — musste vier Zeilen dazubekommen, weil sie als einzige nichts dekodierte; ohne diese Änderung hätte kein Sitzungstest laufen können.

**E-228 — Die Neuvergabe prüft, wem die entfernte Zeile gehörte.**
*Kontext:* `replaceSession` löscht die alte Zeile über ihren Token-Hash und legt die neue an. Beides in einer Transaktion erfüllt S-FIX-1. Wer den Nutzer der neuen Zeile bestimmt, ist damit noch nicht gesagt.
*Verworfen:* Der übergebenen `userId` vertrauen, weil sie ohnehin aus der aufgelösten Sitzung stammt.
*Grund:* „Stammt ohnehin aus" ist die Formulierung, die in zehn der dreiunddreißig Advisories vor dem Fehler stand. Ein `DELETE` der Sitzung von A plus ein `INSERT` für B ist eine Eigentümerumschreibung — dieselbe Wirkung wie das `UPDATE`, das Trigger und Torprüfung verbieten, nur über zwei Anweisungen verteilt, an denen beide Prüfungen vorbeisehen. Die Prüfung kostet nichts: Die gelöschte Zeile liefert ihren `user_id` ohnehin zurück.
*Preis:* `replaceSession` kennt jetzt einen Fehlerfall, der nur bei einem Programmierfehler eintritt, und dieser Fehler ist keine `VelveError` — er hat keinen Code für außen, weil er außen nichts zu suchen hat. Wer ihn sieht, hat einen Bug, keine abgelehnte Anfrage.

**E-229 — Der S-FIX-2-Scanner sieht die Sitzungstabelle dieses Repositorys nicht.**
*Kontext:* `pnpm check:session-owner` sucht nach `update` … `session` … `set` … `user_id` in einer Anweisung. Der Schemaname ist konfigurierbar (Option `schema`, Vorgabe `velve`), also steht in der Anweisung `UPDATE ${table} SET …` — und `${table}` enthält das Wort `session` nicht. Der Scanner findet dieses Repository deshalb nicht, weder im Guten noch im Schlechten.
*Verworfen:* (a) Den Tabellennamen wörtlich `velve.session` schreiben, damit der Scanner ihn sieht. (b) Den Scanner erweitern.
*Grund:* (a) gäbe die Konfigurierbarkeit des Schemas auf, die 3.15 A.2 ausdrücklich vorsieht, und wäre auch inhaltlich falsch: Das Repository schreibt in das Schema, das ihm gegeben wurde. (b) wäre richtig, aber `tools/` gehört diesem Feature nicht — die Eigentumsregel verbietet die Änderung, und die Erweiterung ist nicht trivial, weil der Scanner dann der Interpolation folgen müsste. Es kommt hinzu, dass der Scanner die Frage ohnehin nicht scharf beantworten kann: Sein Muster trifft `SET … WHERE user_id = $1` genauso wie `SET user_id = $1`, kann also eine berechtigte Eigentümerbedingung nicht von einer Umschreibung unterscheiden. Getragen wird S-FIX-2 hier vom Datenbank-Trigger (E-23), von `test/db-static-sql.test.ts` und von der Prüfung aus E-228.
*Preis:* Eine Torprüfung, die für den einzigen Ort im Paket, der wirklich in `velve.session` schreibt, grün meldet, ohne ihn gelesen zu haben. Das ist genau die Sorte Prüfung, vor der §5 der Repositoryregeln warnt — „found nothing" von „found a fault" nicht unterscheidbar —, und es steht hier, damit die Erweiterung des Scanners auf interpolierte Tabellennamen nicht vergessen wird.

**E-230 — Die Sitzungsliste zeigt nur, was noch gilt.**
*Kontext:* `session.list` liefert die Sitzungen des Nutzers. Abgelaufene Zeilen stehen bis zum nächsten `sweep` (L-11) weiter in der Tabelle.
*Verworfen:* Alle Zeilen listen und die Anwendung filtern lassen.
*Grund:* Die Liste ist die Oberfläche „hier sind deine angemeldeten Geräte". Eine abgelaufene Zeile darin liest sich wie ein Gerät, das noch angemeldet ist, und lädt zum Widerruf einer Sitzung ein, die es nicht mehr gibt. Die Frage, ob eine Sitzung gilt, wird an genau einer Stelle beantwortet — den beiden Fristen —, und die Liste stellt sie genauso wie die Auflösung.
*Preis:* Die Liste hängt vom Zeitpunkt ab, nicht nur vom Bestand: Dieselbe Zeile verschwindet ohne `DELETE`. Wer die Tabelle direkt zählt, sieht mehr Zeilen als die Liste zeigt, und das muss dokumentiert sein, sonst sieht es nach einem Fehler aus.

**E-231 — Die Uhr ist eine Pflichtangabe des Sitzungsdienstes, keine Vorgabe.**
*Kontext:* 3.15 A.2 nennt `clock` mit der Systemuhr als Vorgabe. Der naheliegende Weg wäre `options.clock ?? { now: () => new Date() }` gewesen.
*Verworfen:* Eine eingebaute Systemuhr als Rückfallwert.
*Grund:* Zwei Gründe, und der erste war eine Prüfung, die schon stand: `test/keys-static-scan.test.ts` verbietet `new Date(` in `src/core` überhaupt, damit kein Geheimnis aus einer Uhr entsteht. Der zweite ist der bessere: Eine Vorgabe hätte eine zweite Zeitquelle geschaffen, die niemand konfiguriert und niemand sieht. Die Frische-Prüfung im HTTP-Pfad liest `environment.clock`; hätte der Sitzungsdienst still seine eigene Uhr benutzt, wären es zwei — und ein Test, der die eine stellt, hätte die andere nicht bewegt. Wer den Kern baut, gibt die Uhr; die Vorgabe entsteht eine Schicht weiter oben, wo `new Date()` erlaubt ist.
*Preis:* Ein Pflichtfeld mehr in einer internen Fabrik. Der Aufrufer, der `createVelveAuth` zusammensetzt, muss die Systemuhr ausdrücklich einsetzen — und wenn er es vergisst, ist es ein Typfehler und keine falsche Zeit.

**E-232 — Die Auflösung bringt die Datenbankuhr mit.**
*Kontext:* Die Leerlauffrist wird höchstens einmal je `idleWriteInterval` geschrieben. Die erste Fassung schickte die `UPDATE`-Anweisung bei jeder Anfrage los und ließ die Bedingung `last_used_at <= now() - interval` entscheiden — zwei Anweisungen je Anfrage, auch wenn nichts zu schreiben war.
*Verworfen:* (a) Zwei Anweisungen je Anfrage in Kauf nehmen. (b) Die Fälligkeit gegen die Uhr des Anwendungsprozesses prüfen.
*Grund:* (a) verdoppelt die Last des heißesten Pfades der Bibliothek und verfehlt die Schwelle aus T-CACHE-1, die für *n* Anfragen genau *n* Auflösungsabfragen verlangt. (b) hätte zwei Uhren verglichen: Geht die Prozessuhr gegenüber der Datenbank nach, gilt der Schreibvorgang nie als fällig, und eine täglich benutzte Sitzung liefe nach sieben Tagen ab. Die Auflösungsabfrage liefert deshalb `now() AS observed_at` mit — dieselbe Abfrage, eine Spalte mehr —, und beide Seiten des Vergleichs stammen aus derselben Uhr. Die Bedingung bleibt zusätzlich in der `UPDATE`-Anweisung, damit zwei gleichzeitige Anfragen nicht beide schreiben.
*Preis:* Die Auflösungsabfrage weicht um eine Spalte von dem Wortlaut ab, den 3.5 vorgibt. T-CACHE-2 vergleicht dieses SQL byteweise mit einem Fixture; die Abweichung ist damit eine bewusste Entscheidung, die im Fixture sichtbar wird — und genau so ist die Prüfung gemeint.

**E-233 — Die Frische wird dort erzwungen, wo der Aktor entsteht.**
*Kontext:* B.9 verlangt Frische für `session.list`, `revoke`, `revokeAllOther` und `revokeAll`. Die HTTP-Schicht prüft sie bereits für jede Route mit `freshness: "required"`. Für den direkten Servermethodenaufruf prüft sie dort niemand.
*Verworfen:* Sich auf die Prüfung in der Pipeline verlassen und im Kern nichts tun.
*Grund:* Die Pipeline prüft, was die Routendeklaration sagt; eine Methode, die morgen ohne Route aufgerufen wird, hat keine Deklaration. Die Prüfung sitzt deshalb dort, wo sie nicht zu umgehen ist: `actorOfFreshSession` prüft die Frische und gibt erst dann den Aktor heraus. Wer eine dieser Operationen schreibt, braucht den Aktor — und bekommt ihn nur mit der Prüfung. Vergessen ist keine Option mehr, es ist ein Kompilierfehler.
*Preis:* Die Frische wird auf dem HTTP-Weg zweimal geprüft, einmal in der Pipeline und einmal hier. Beide lesen dieselbe Uhr, aber das Fenster steht an zwei Stellen: `HttpEnvironment.freshnessWindowInSeconds` und `SessionSettings.freshnessWindowMs`. Wer die Instanz zusammensetzt, muss das eine aus dem anderen ableiten; tut er es nicht, gelten zwei Fenster. Das gehört in die Aufbaufunktion, die es noch nicht gibt, und ist hiermit vermerkt.

**E-234 — Für den Kennwort-Reset prägt dieses Feature keinen Aktor.**
*Kontext:* S-FIX-6 verlangt, dass der Kennwort-Reset alle Sitzungen widerruft. Beim Reset gibt es keine Sitzung: Der Nachweis ist ein eingelöstes Einmal-Token. Jede Repository-Methode, die Zeilen über ihren Eigentümer erreicht, verlangt aber einen `Actor` — auch `deleteEverySessionOwnedBy`, die hier gebraucht wird —, und der einzige Erzeuger eines `Actor` ist laut E-93 die Sitzungsauflösung. (Korrektur: Der ursprüngliche Wortlaut behauptete, **jede** Methode auf `velve.session` verlange einen Aktor; das stimmt nicht, siehe E-242.)
*Verworfen:* (a) Eine zweite Fabrik `actorOfUserId(userId)` im Sitzungsmodul. (b) Eine Repository-Methode ohne `actor`, die nur eine `userId` nimmt.
*Grund:* (a) wäre genau das Loch, das E-93 zumauern soll — eine Zeichenkette rein, ein Aktor raus, und niemand sieht der Signatur an, woher die Zeichenkette kam. (b) bräche S-OWNER-1, das keine Methode ohne `actor` auf einer Tabelle mit `user_id` zulässt. Also verlangt `revokeEverySessionOfUser` einen `Actor` und prägt keinen: Das Feature, das Einmal-Token einlöst, bringt ihn mit, und wenn es dafür keinen rechtmäßigen Weg hat, wird das dort sichtbar statt hier verdeckt.
*Preis:* Die Anforderung ist damit nur zur Hälfte in diesem Feature erfüllbar. Der Reset-Pfad braucht einen zweiten rechtmäßigen Aktor-Erzeuger — Provenienz „eingelöstes Einmal-Token" statt „aufgelöste Sitzung" —, und den muss die Welle bauen, der `one_time_token` gehört. Bis dahin steht die Fähigkeit bereit und ruft sie niemand auf.

**E-235 — Warum eine Sitzung nicht gilt, erfährt auch das Protokoll nicht.**
*Kontext:* 3.15 F.1 führt unter `session_required` vier innere Gründe: `cookie_absent`, `session_not_found`, `session_idle_expired`, `session_absolute_expired`. S-ENUM-6 verlangt, dass der wahre Grund serverseitig protokolliert wird.
*Verworfen:* Zuerst ohne Fristfilter suchen und die Fristen in TypeScript auswerten, um die drei Gründe unterscheiden zu können.
*Grund:* Dann entschiede nicht mehr die Abfrage, ob eine Sitzung gilt, sondern eine Verzweigung dahinter — und S-CACHE-2 verlangt wörtlich die eine Abfrage mit allen vier Bedingungen. Eine Abfrage, die abgelaufene Zeilen zurückgibt, ist außerdem eine Abfrage, deren Ergebnis jemand versehentlich als Sitzung behandeln kann; genau diese Sorte Beinahe-Sitzung stand hinter dem schwersten Fehler des Vergleichssystems. Die Unterscheidung ist ein Protokolldetail, die Entscheidung ist eine Sicherheitseigenschaft.
*Preis:* Ein Treffer, der keiner ist, heißt für das Protokoll immer `session_not_found`. Wer wissen will, ob Sitzungen an der Leerlauf- oder an der absoluten Frist sterben, kann das aus den Protokollen nicht ablesen und muss die Tabelle vor dem Aufräumen zählen.

**E-236 — `actorOfResolvedSession` nimmt jetzt nur noch, was die Auflösung erzeugt.**
*Kontext:* E-93 hat die Lücke beschrieben und die Änderung wörtlich an diese Welle übergeben: Der Parameter wird vom strukturellen `{ userId: string }` auf den nominalen `ResolvedSession` gezogen, und der Test, der die alte Gestalt festhielt, wird umgedreht.
*Verworfen:* (a) Die Marke auf den vollen Rückgabewert der Auflösung legen, also `{ session, user }` aus 3.15 B.2. (b) Die Änderung aufschieben, bis die Instanz zusammengesetzt wird.
*Grund:* (a) scheiterte an einer Tatsache dieser Welle: Den Typ `User` gibt es noch nicht, er gehört einem anderen Feature, und ein hier erfundener `User` wäre beim Zusammenführen ein zweiter. Die Marke bleibt deshalb auf `{ userId }`, und was die Auflösung zusätzlich mitgibt — die `Session` — hängt als Schnittmenge daran (`SessionResolution`). (b) hätte die Lücke offen gelassen, während zum ersten Mal echte Aufrufer entstehen; genau davor warnt der Preis in E-93.
*Preis:* Sechs fremde Testdateien mussten mit. Sie brauchten alle einen Aktor für einen Nutzer, den sie selbst angelegt hatten, und bekamen ihn bisher aus einem Objektliteral. Statt sechs verstreuter Typzusicherungen steht die Behauptung jetzt einmal in `test/db-fixtures.ts` als `actorOfTestUser` — sichtbar, benannt und mit dem Hinweis, dass in der Bibliothek nur die Auflösung das darf. Die Eigentumsregel ist damit an sechs Stellen überschritten worden; das war die Bedingung dafür, den Auftrag aus E-93 überhaupt auszuführen.

**E-237 — S-FIX-2 wird hier an den ausgeführten Anweisungen geprüft, nicht am Quelltext.**
*Kontext:* E-229 hielt fest, dass die Torprüfung dieses Repository nicht sieht, weil der Tabellenname interpoliert ist. Ein Vermerk allein lässt die Lücke offen.
*Verworfen:* Es beim Vermerk belassen und auf Trigger und `test/db-static-sql.test.ts` vertrauen.
*Grund:* Beide prüfen etwas anderes. Der Trigger prüft zur Laufzeit und nur, was tatsächlich ausgeführt wird; die Quelltextprüfung prüft Zeichenketten mit `${table}` darin. Was fehlte, war die Anweisung so, wie sie an die Datenbank geht. Das Repository gibt sie her: ein Treiber, der nur mitschreibt, jede Methode einmal aufgerufen, und die zehn entstehenden Anweisungen liegen mit eingesetztem Schema vor. Geprüft wird daran die Eigenschaft, um die es geht — welche Spalten eine Anweisung **zuweist**, nicht welche sie filtert.
*Preis:* Dabei ist herausgekommen, dass das Muster des Tors die legitime Verlängerung der Leerlauffrist als Umschreibung meldet: `UPDATE … SET last_used_at = … WHERE id = $1 AND user_id = $2` trifft `update` … `session` … `set` … `user_id`, obwohl `user_id` nur in der Bedingung steht. Sähe der Scanner diese Datei, bliebe das Tor rot — und die Datei ist nicht falsch, das Muster ist zu grob. Der Befund steht als eigener Testfall im Feature, damit er sichtbar ist und nicht als Zufall durchgeht; das Muster gehört `tools/`, und das gehört diesem Feature nicht. Wer es schärft, trennt die Zuweisungsliste zwischen `SET` und `WHERE` vom Rest, so wie es dieser Test tut.

**E-238 — Frische entscheidet die Uhr, von der `created_at` stammt.**
*Kontext:* Der Review hat gemessen, was E-231 offengelassen hatte: `created_at` und beide Fristen schreibt die Datenbank, die Frische verglich aber die Prozessuhr gegen `created_at`. Geht die Prozessuhr 16 Minuten vor, wird eine Sitzung abgelehnt, die die Datenbank gerade eben angelegt hat; geht sie eine Stunde nach, passiert eine 50 Minuten alte Sitzung das Fenster. Die zweite Richtung öffnet das Tor, das die Operationen an Anmeldedaten schützt.
*Verworfen:* (a) Die Prozessuhr behalten und die Abweichung als Betriebsproblem dokumentieren. (b) Beide Uhren prüfen und die strengere gewinnen lassen.
*Grund:* (a) verlegt eine Sicherheitseigenschaft in die Betriebsanleitung; NTP-Ausfall und virtualisierte Uhren sind der Normalfall, nicht die Ausnahme. (b) hätte zwei Zeitquellen behalten und die Frage „welche gilt" nur verschoben. E-232 hatte dieselbe Frage für den Leerlaufschreibvorgang schon entschieden und die Antwort mitgeliefert: Die Auflösungsabfrage bringt `now()` als `observedAt` mit. Genau diese Spalte lag an der Stelle bereit, an der die Frische entschieden wird, und wurde nicht gelesen. Jetzt trägt `SessionResolution` sie, und `assertSessionIsFresh` bekommt sie als `now`.
*Preis:* Die Option `clock` ist damit im Sitzungsmodul unbenutzt. Sie bleibt in den Optionen — die Instanz reicht dieselbe Uhr an alle Module —, aber sie ist hier eine Angabe ohne Wirkung, und das ist eine Falle: Wer im Test die Uhr stellt, macht eine Sitzung nicht mehr alt. Das steht jetzt fett in der Dokumentation, und drei eigene Tests mussten umgestellt werden, weil genau sie in diese Falle gelaufen waren — sie altern die Sitzung jetzt dort, wo `created_at` steht. Ein fremder Testfall des Reviews wurde aus demselben Grund um eine Spalte erweitert; seine Behauptung ist unverändert. **Nachtrag:** Dieser Absatz beschreibt einen Zustand, den es nicht mehr gibt. Die Option ist mit **E-247** ganz entfallen, weil eine Angabe ohne Wirkung genau die Falle bleibt, die hier als Preis in Kauf genommen wurde; wer diesen Eintrag allein liest, liest ihn falsch.

**E-239 — Eine Neuvergabe, die nichts ersetzt, schlägt fehl.**
*Kontext:* `replaceSession` löschte die alte Zeile und legte die neue an, auch wenn das `DELETE` keine Zeile traf. Der Review hat zwei Neuvergaben derselben Sitzung gleichzeitig laufen lassen: Beide erfüllten sich, und wo eine Sitzung war, waren zwei. Der Verlierer löscht nichts, weil der Gewinner schon festgeschrieben hat, und fügt trotzdem ein.
*Verworfen:* (a) Es dabei belassen, weil beide Zeilen demselben Nutzer gehören. (b) Eine getrennte Methode für den Fall „ohne Vorgänger".
*Grund:* (a) verkennt, worum es bei einer Neuvergabe geht: Sie ist der Schnitt, an dem die alte Vertrauensstufe endet. Zwei lebende Sitzungen nach einem Faktorwechsel heißt, dass eine davon nie in die Neuvergabe einbezogen wurde — genau die Sitzungsvermehrung, die S-FIX-1 ausschließt. (b) war unnötig: Für den Fall ohne Vorgänger gibt es die Methode schon, sie heißt `insertSession`, und der Dienst nennt sie `issue`. Wer einen `previousTokenHash` übergibt, behauptet damit, es gebe einen Vorgänger; trifft das nicht zu, ist die Behauptung falsch und die Operation gescheitert.
*Preis:* Ein Wettlauf endet jetzt für einen der beiden mit einem Fehler statt mit einer Sitzung. Der Dienst übersetzt ihn nach `session_required` — die Sitzung, auf die sich der Aufrufer berief, gibt es nicht mehr, und die richtige Antwort ist neu anmelden. Ein eigener Test, der die alte Nachsicht festhielt („issues a session even when the previous token is already gone"), stand genau daneben und ist umgedreht; er war die Stelle, an der die Lücke sichtbar gewesen wäre.

**E-240 — `isCurrent` ist außerhalb von `session.list` falsch, nicht wahr.**
*Kontext:* 3.15 C führt `isCurrent: boolean` mit dem Zusatz „nur in `session.list` gesetzt". Das Repository setzte es überall dort auf `true`, wo die zurückgegebene Sitzung tatsächlich die aufrufende ist — beim Anlegen und beim Auflösen.
*Verworfen:* `true` zu lassen, weil es an diesen beiden Stellen ja stimmt.
*Grund:* Es stimmt, und es ist trotzdem falsch. Das Feld beantwortet die Frage „ist das die Sitzung, aus der ich gerade frage" **innerhalb einer Liste**; außerhalb einer Liste gibt es keine Alternativen, gegen die verglichen würde. Ein `true`, das immer `true` ist, sieht aus wie eine Information und ist keine — und die erste Anwendung, die es außerhalb der Liste ausliest und daraus etwas schließt, schließt aus einer Konstanten. `false` ist der Wert, den die Vorgabe verlangt, und er ist auch der ungefährlichere, weil er zu nichts einlädt.
*Preis:* Die Sitzung, die `resolve` zurückgibt, sagt von sich selbst, sie sei nicht die aktuelle. Das liest sich beim ersten Mal falsch, und es steht deshalb in der Referenz. Wer wirklich vergleichen will, hat die Sitzungs-ID der Auflösung ohnehin in der Hand.

**E-241 — Das Abmelden nennt keinen Eigentümer, weil das Token einer ist.**
*Kontext:* Mit E-141 sucht `test/db-static-sql.test.ts` die Eigentümerbedingung nur noch **vor** dem `RETURNING`. Damit fiel auf, was vorher versehentlich durchging: `DELETE FROM ${table} WHERE token_sha256 = $1 RETURNING id, user_id` erfüllte die Regel nur, weil `user_id` hinter `RETURNING` stand. Die Prüfung war richtig und der Treffer verdient.
*Verworfen:* (a) `AND user_id = $2` ergänzen. (b) Vor dem Löschen auflösen und dann mit Aktor löschen.
*Grund:* (a) ist nicht möglich: `signOut` bekommt ein Token und sonst nichts; wer der Nutzer ist, steht erst in der Zeile, die gelöscht werden soll. (b) wäre genau das vorgelagerte `SELECT`, das S-OWNER-2 verbietet, und es machte aus einer Anweisung zwei mit einem Fenster dazwischen. Der Punkt ist ein anderer: Ein Sitzungstoken **ist** der Nachweis. Wer es vorlegt, hat die Sitzung; ein zusätzliches `user_id`-Prädikat prüfte nichts, was der Hash nicht schon geprüft hat. Hier stand ursprünglich weiter: „Das ist dieselbe Begründung, die das Einlösen eines Einmal-Tokens schon als benannte Ausnahme trägt, und sie steht jetzt als zweite benannte Ausnahme daneben — mit ihrem Grund, nicht als Lücke im Muster." Der Satz war falsch, und wie falsch, steht im *Preis*.
*Preis:* **Korrektur nach E-142:** Die Ausnahme wurde zunächst als zweiter benannter regulärer Ausdruck in `test/db-static-sql.test.ts` eingetragen, und dieser Eintrag hat das als Zugewinn beschrieben. Er war beides nicht. Der Ausdruck `/DELETE\s+FROM[\s\S]*WHERE\s+token_sha256\s*=/i` lief über **jedes** SQL-Literal in `src/`, nicht über Sitzungs-SQL, und hätte jedem Modul auf jeder Tabelle den Verzicht erlaubt, sobald irgendwo ein `token_sha256`-Prädikat im Statement stand — hinter `RETURNING` eingeschlossen, weil er nicht verankert war. Der Kommentar beschrieb einen engen Einzelfall, das Muster gewährte eine Klassenausnahme; genau die Sorte Prüfung, vor der §5 der Repositoryregeln warnt. `main` hat den Mechanismus inzwischen ersetzt: Die Anweisung trägt die Markierung `-- no owner predicate: S-FIX-3` in ihrem eigenen Text, sie überlebt die Interpolation des Schemas, und sie zwingt den Autor, die Anforderung zu nennen, statt sie sich von einer Liste an anderer Stelle gewähren zu lassen. Der Preis ist jetzt eine Zeile im Statement — und dass die Begründung dort steht, wo sie gelesen wird.

**E-242 — Vier Repository-Methoden kommen ohne Aktor aus, und das ist keine Nachlässigkeit.**
*Kontext:* Der Review hat eine falsche Behauptung im *Kontext* von E-234 gefunden: Dort steht, jede Methode auf `velve.session` verlange einen `Actor`. Vier tun es nicht — `insertSession`, `findSessionByTokenHash`, `deleteSessionByTokenHash` und `replaceSession`. Der Satz war im guten Glauben geschrieben und ist trotzdem falsch; er ist oben korrigiert, und der Grund steht hier.
*Verworfen:* (a) Den vier Methoden einen `actor` anhängen, damit die Behauptung stimmt. (b) Es bei der falschen Behauptung belassen und die Ausnahmen unerwähnt lassen.
*Grund:* (a) wäre ein Parameter, den keine dieser Methoden benutzen könnte. `insertSession` legt die erste Zeile eines Nutzers an — es gibt keinen Aktor, der ihr vorausgehen könnte, denn der Aktor entsteht erst aus einer Sitzung. Die anderen drei sind über `token_sha256` adressiert, und der Hash ist ein stärkeres Prädikat als der Eigentümer: Wer das Token vorlegt, hat die Sitzung; ein zusätzliches `user_id = $2` prüfte nichts, was der Hash nicht schon geprüft hat, und `signOut` könnte es gar nicht liefern (E-241). Die Regel, die tatsächlich gilt, ist enger und schärfer als die falsche: **Jede Methode, die Zeilen über ihren Eigentümer erreicht, verlangt einen Aktor; wer sie über ein Geheimnis erreicht, weist damit bereits nach.** (b) hätte eine Prüfregel auf einer Unwahrheit stehen lassen.
*Preis:* Die Regel hat jetzt zwei Formen, und nur die längere ist wahr. Wer sie mechanisch prüfen will — T-OWNER-1 will das —, muss „über den Eigentümer erreichbar" von „über ein Geheimnis adressiert" unterscheiden können, und das steht in keinem Typ. Bis dahin trägt sie ein benannter Grund je Ausnahme, hier und in `test/db-static-sql.test.ts`.

**E-243 — Dass ein Kennwortwechsel die anderen Sitzungen mitnimmt, hängt am Namen der aufgerufenen Methode.**
*Kontext:* `reissue` und `reissueAfterCredentialChange` haben dieselbe Gestalt und unterscheiden sich in der Wirkung: Die eine ersetzt eine Sitzung, die andere alle. Ein Kennwortwechsel, der versehentlich `reissue` aufruft, erfüllt S-FIX-1 und verliert S-FIX-6 — still, und niemand hier kann es bemerken.
*Verworfen:* (a) Die beiden Methoden zu einer mit einem Schalter zusammenlegen. (b) `reissue` verbieten, wenn der Nutzer weitere Sitzungen hat.
*Grund:* (a) ist genau der Schalter, den S-FIX-6 ausschließt („Das ist kein Schalter."), und Regel 1 der Schnittstelle verbietet den booleschen Parameter ohnehin. (b) wäre falsch: Der Abschluss des zweiten Faktors ist eine Neuvergabe **ohne** Widerruf der anderen Sitzungen und der häufigste Aufrufer von `reissue`. Der Aufrufer, der es richtig machen muss, ist das Kennwort-Feature, und der Aufruf steht dort. Diese Übergabe wird deshalb benannt, wie E-233 und E-234 benannt sind: **`password.change` und `password.set` rufen `reissueAfterCredentialChange` auf, `password.redeemReset` ruft `revokeEverySessionOfUser` auf, und kein Kennwortpfad ruft `reissue`.**
*Preis:* Eine Anforderung, die von einer Namenswahl an einer fremden Aufrufstelle abhängt. Prüfbar wird sie erst, wenn es die Aufrufstelle gibt — dann als Test des Kennwort-Features: Kennwort ändern, zweite Sitzung vorher anlegen, danach zählen. Bis dahin ist das hier die einzige Stelle, an der steht, was dort stehen muss.

**E-244 — Eine abgeschwächte Vorgabe meldet die Instanz, nicht der Sitzungsdienst.**
*Kontext:* S-DEFAULT-1 verlangt, dass eine Einstellung, die eine Vorgabe abschwächt, beim Start protokolliert wird. `sessionMetadata: "full"` ist eine solche Einstellung: Sie hebt die Datenminimierung aus L-10 auf. Protokolliert wird sie nirgends.
*Verworfen:* Dem Sitzungsdienst eine Protokollsenke geben.
*Grund:* Es gäbe dann zwei — `HttpEnvironment.log` existiert bereits und ist die Senke der Instanz. Zwei Senken heißt zwei Formate, zwei Konfigurationen und zwei Stellen, an denen ein Betreiber suchen muss. Der Startzeitpunkt gehört ohnehin der Aufbaufunktion: Sie liest die Konfiguration, sie kennt die Vorgaben, und sie hat die Senke. Der Sitzungsdienst liefert ihr die Grundlage, indem er die gewählte Betriebsart nicht versteckt.
*Preis:* Bis es diese Aufbaufunktion gibt, ist S-DEFAULT-1 für `sessionMetadata` unerfüllt, und niemand sieht es. Das ist die dritte Übergabe dieser Art nach E-233 und E-243; alle drei enden an derselben Stelle, nämlich dort, wo `createVelveAuth` entstehen wird.

**E-245 — Vier Exporte ohne Aufrufer bleiben, und der Prüfplan hält fest, welche.**
*Kontext:* `knip` behandelt jede Testdatei als Einstiegspunkt, also gilt ein Export, den nur ein Test aufruft, als benutzt. `DEFAULT_SESSION_CONFIG`, `InvalidSessionConfigError`, `isSessionFresh` und `createSessionService` haben in `src/` keinen Aufrufer außerhalb ihrer eigenen Datei.
*Verworfen:* Sie einzeln zu entfernen oder nicht zu exportieren, bis ein Aufrufer da ist.
*Grund:* Alle vier sind die Schnittstelle des Moduls nach oben, und oben gibt es noch nichts: `createVelveAuth` ist die Aufbaufunktion, die sie aufrufen wird. Sie jetzt zu verstecken hieße, sie beim Zusammenbau wieder herauszuholen — Bewegung ohne Erkenntnis. `isSessionFresh` ist zusätzlich die Definition, auf der `assertSessionIsFresh` sitzt; die Prädikatsform ohne Ausnahme ist das, was eine Oberfläche braucht, die „diese Sitzung ist frisch" anzeigen will, statt es zu erfragen und zu fangen.
*Preis:* Vier Exporte, die das Tor nicht als tot erkennt, weil Tests sie am Leben halten. Der Review hat die Liste als Prüffall festgeschrieben, damit sie eine Entscheidung bleibt und nicht zum Sammelbecken wird: Wer einen fünften hinzufügt, muss die Liste ändern und dabei sagen, warum.

**E-246 — Die Neuvergabe nach einem Anmeldedatenwechsel prüft die Frische nicht selbst.**
*Kontext:* `list`, `revoke`, `revokeEveryOther` und `revokeEvery` holen ihren Aktor über `actorOfFreshSession` und prüfen damit die Frische (E-233). `reissueAfterCredentialChange` tut es als einzige Methode mit Auflösung nicht.
*Verworfen:* Die Prüfung dort ebenfalls einzubauen, der Einheitlichkeit wegen.
*Grund:* B.9 stellt die Forderung an `password.set` und `password.change`, also an die Route, **bevor** gearbeitet wird. Die Neuvergabe ist die Folge dieser Arbeit und läuft danach — nach einem Argon2id-Durchlauf, der Zehntelsekunden kostet. Eine Frische-Prüfung an dieser Stelle könnte also fehlschlagen, **nachdem** das Kennwort schon geändert ist: Das neue Kennwort gilt, die anderen Sitzungen leben weiter, und der Aufrufer hat keine neue Sitzung — genau der halbe Zustand, den S-FIX-6 ausschließt. Eine Prüfung, die spät genug scheitert, um Schaden anzurichten, ist schlechter als keine.
*Preis:* Die Regel „die Frische wird geprüft, wo der Aktor entsteht" hat eine Ausnahme, und sie steht nur hier. Wer `reissueAfterCredentialChange` aus einem Pfad aufruft, den B.9 nicht ohnehin auf Frische verpflichtet, umgeht die Prüfung — und das ist ein Grund mehr, dass die Aufrufliste aus E-243 vollständig bleibt.

**E-247 — Der Sitzungsdienst nimmt gar keine Uhr mehr.**
*Kontext:* E-238 hat die Frische auf die Datenbankuhr gelegt und damit die letzte Verwendung der Option `clock` in diesem Modul entfernt. Der Eintrag hat sie trotzdem behalten, damit eine Instanz allen Modulen dieselbe Uhr reichen kann, und den Preis als Dokumentationsaufgabe beschrieben.
*Verworfen:* Die Option als angenommen-und-ignoriert stehen lassen, mit einem fetten Absatz in der Referenz.
*Grund:* Ein Parameter, den es gibt, der typprüft und der verworfen wird, ist ein Angebot, das lügt. Wer eine Testuhr übergibt, bekommt einen Dienst, der `now()` aus der Datenbank liest; ein Test, der diese Uhr vorstellt, um eine Sitzung altern zu lassen, beobachtet nichts und wird **aus dem falschen Grund grün**. Das ist keine Vermutung: Genau in diese Falle waren drei eigene Tests gelaufen, bevor E-238 sie sichtbar machte, und ein Absatz in der Referenz hätte sie nicht davor bewahrt — ein Übersetzungsfehler auf `clock:` schon. Die Bequemlichkeit des Aufrufers verlangt nicht, dass dieses Modul einen Parameter annimmt, den es wegwirft; die Aufbaufunktion reicht jedem Modul, was es liest.
*Preis:* Die Abwesenheit ist jetzt selbst die Invariante und muss als solche dokumentiert sein, sonst sucht der nächste Leser die Option. Zwei Prüffälle des Reviews, die eine schiefe Prozessuhr einspeisten, können das nicht mehr — sie behaupten stattdessen, was jetzt strukturell gilt, und ihre Namen sagen es. Verschwunden ist damit auch die Möglichkeit, eine Uhrenabweichung überhaupt zu simulieren; gäbe es je einen Zeitpunkt, den die Datenbank nicht liefert, käme die Uhr als Pflichtfeld zurück und mit ihr diese Entscheidung.

**E-248 — Die Markierung nennt die Anforderung, von der abgewichen wird, und die Bedingung, unter der das zulässig ist.**
*Kontext:* Die Markierung am Abmelde-`DELETE` zitierte zunächst S-FIX-3, weil dort eine Sitzungszeile allein über `token_sha256` adressiert wird. Der Tor-Agent hat zwei Dinge daran auseinandergenommen. Erstens regelt S-FIX-3 die acht Ereignisse, die die Vertrauensstufe ändern — das Abmelden ist keines davon; wer der Zitatnummer folgt, landet bei einer Klausel, die von Abmeldung nicht spricht. Zweitens riss die Markierung als Zeilenkommentar hinter `DELETE FROM ${table}` eine Falle auf: Fällt der Zeilenumbruch weg — Protokollierung, Weiterreichung, Normalisierung —, verschluckt der Kommentar das `WHERE`, und übrig bleibt `DELETE FROM velve.session`. Alle Zeilen.
*Verworfen:* (a) Bei S-FIX-3 bleiben, weil die Leseart des Hashes als Adresse von dort stammt. (b) Die Markierung ans Ende hinter `RETURNING` setzen, wo sie nichts verschlucken kann.
*Grund:* (a) verwechselt Grund und Abweichung. Abgewichen wird von **S-OWNER-2** — die Eigentümerbedingung gehört ins Prädikat —, und die Rechtfertigung ist, dass die einzige Fassung, die S-OWNER-2 wörtlich erfüllte, ein vorgelagertes `SELECT` wäre, das dieselbe Anforderung an anderer Stelle verbietet. Der Konflikt **ist** die Begründung; S-FIX-3 liefert nur die Lesart, dass der Hash die Adresse ist. Zitiert wird deshalb die Abweichung, nicht die Lesart. (b) wäre an dieser einen Stelle sicher und anderswo wieder nicht. Ein Blockkommentar endet, wo er endet, egal wie die Leerzeichen normalisiert werden — die Eigenschaft, die man haben will, hängt dann nicht mehr an der Position.
*Preis:* Die Zulassungsbedingung, die aus der verworfenen Zwei-Ausnahmen-Grenze übrig bleibt, steht nur in Prosa: **Eine Markierung ist zulässig, wenn das Prädikat selbst ein Geheimnis ist** — der Token-Hash hier, der Einmal-Token dort. Sie lässt sich nicht mechanisch prüfen; was die Prüfung sieht, ist nur, dass eine Anforderung genannt wurde. Wer eine dritte Markierung setzt, muss diesen Nachweis führen, und wer sie liest, muss ihn verlangen.
