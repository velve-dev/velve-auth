...
**Price.** Until that branch runs, the shipped library contradicts three clauses of its own binding specification. What catches a third exempt route is stronger than this entry first said: `test/auth-route-table.test.ts` reads the **live route table** and fails for one declared in any file, and `test/oauth-callback-surface.test.ts` counts the string in the tree beside it. Both are boundaries rather than requirements, and neither says which number the specification means. If the amendment does not happen, this entry is the record that it was promised.
...
### E-582 signed the phone out, and the constant said it would not
...
### E-586's first and fourth reasons are not about Apple
...
**Price.** A new test file belongs to no feature's partition, and that is the one case §5's rules do not cover: a partition is cut over the files that exist when the wave starts, so a file created mid-wave is owned by nobody by construction rather than by oversight. It is the same ownership gap this file was created to avoid, one size smaller. And the third case relies on the owner predicate to make another account's row indistinguishable from a row that never existed, so it would pass for the wrong reason if the predicate were ever dropped.
...
**Price.** `boundTo` is a general escape hatch on a service three features are editing, and it can be used to put a long operation inside someone else's transaction. What keeps §7's warning about a lock held across a third party's timeout from applying here is that both outbound calls finish before this transaction opens — a property of the ordering and not of the type, so E-973 buys it a test rather than leaving it a note. The edit that would break it is a fetch moved *inside* `linkIdentityAndReissue`'s transaction, not one moved below the call site; and the same file already does the thing this Price warns about, because `createAccountFor` awaits two hooks inside `accountForSignIn`'s transaction.
