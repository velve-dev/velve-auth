...
**Context.** `BaseConfig` in `core/auth/config.ts` had no `oauth` field and no `plugins` field. Two of wave 4's four features need one each, so as it stood both writers had to edit the same file — which §5 forbids outright. PR #28 opened the four route seams and this one was not among them.
...
**Context.** Adding eighteen exported types to `src/index.ts` should change the public surface, and `test/api-surface.test.ts` compares `dist/*.d.mts` against a committed snapshot. Run directly with `npx vitest`, it passed. The snapshot is real and the surface had changed; what it read was a `dist/` built before the change.
**Rejected.** Taking the green run as evidence and moving on, which is what the run looks like.
**Reason.** The test reads `dist/`, and `package.json` puts the build in front of it as a `pretest` hook — so `pnpm test` rebuilds and `npx vitest run test/api-surface.test.ts` does not. Rebuilding first turned it red with exactly the eighteen names, and the snapshot was then updated deliberately, which is what §5 means by the surface not changing unannounced. The test guards `dist/` being absent and cannot guard `dist/` being old; the freshness lives in a lifecycle hook, one file away from the assertion that depends on it.
