...
**Reason.** `nestServerMethods` keys the surface by the dotted `name`, so the name is what can collide, and it is now tested. The id is tested beside it because 3.11 states the prohibition about the plugin's namespace and a plugin that claims `session` as its id has claimed it whether it has written the route yet or not. Both halves were planted: dropping the name half lets three of the probe's cases start, dropping the id half lets seven start, and neither absorbs the other.
