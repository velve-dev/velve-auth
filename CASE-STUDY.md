...
**E-211 — Die Sperr-Erklärung steht in einer eigenen Zeichenkette, weil eine Vorlage sie nur maskiert aufnehmen kann.**
*Kontext:* Seit E-147 verlangt `main`, dass eine Sperre ihr Ziel in einem Blockkommentar nennt: `/* locks: ${schema}.user */`. Die Prüfung streicht `${…}` aus dem gelesenen Ziel, bevor sie es mit `user` vergleicht — `${user}` wird dabei zum leeren Namen und fällt durch, obwohl genau diese Variable die richtige Tabelle hält. Die Anweisung in `sign-in-methods.ts` ist eine Template-Zeichenkette, und darin lässt sich `${schema}` nur als `\${schema}` schreiben; der Rückstrich steht nicht in der Zeichenklasse, mit der `tools/lock-order.mjs` das Ziel liest. Beide Formen wurden gemessen statt überlegt: die maskierte meldet „a row lock that does not declare what it locks", `${user}` meldet „a row lock on ${user}".
*Verworfen:* (a) `/* locks: velve.user */` schreiben — die Prüfung nimmt das an. (b) Die Zeichenklasse in `tools/lock-order.mjs` um den Rückstrich erweitern.
*Grund:* (a) behauptet einen festen Schemanamen, den der Aufruf nicht kennt; das Schema ist konfigurierbar, und eine Erklärung, die nur für die Vorgabe stimmt, ist schlechter als keine. (b) liegt außerhalb des Bereichs dieses Features, und Abschnitt 5 verlangt, so etwas zu melden statt es zu ändern. Bleibt die Erklärung als eigene, nicht interpolierte Zeichenkette hinter der Vorlage: sie steht in derselben Anweisung, reist mit ihr und wird von der Prüfung genau so gelesen, wie sie dasteht.
*Preis:* Eine Verkettung mit `+` in einer Datei, die sonst nur Vorlagen benutzt, und zwei unterdrückte Biome-Regeln in einer Zeile, `noTemplateCurlyInString` und `useTemplate`. Die zweite ist die unangenehme: Biomes eigener Vorschlag ist genau die maskierte Form, die die Prüfung nicht mehr erkennt. Wer die Unterdrückung entfernt und den Vorschlag annimmt, macht die Sperre für die Prüfung unsichtbar, ohne dass sich am ausgeführten SQL etwas ändert.
