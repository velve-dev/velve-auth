...
**Reason.** `src/core/auth/security-options.ts` reads `config.plugins` a third time, to name the configured plugins in the warning line about weakened options. It runs after the registry, takes `id` alone and decides nothing, so the check-and-reading pair E-911 closed is closed — but the list is still indexed twice overall, and the test that counts it says **two** rather than one for that reason. It is not this feature's file and the change is one line, which is exactly the case §5 says to report rather than take.
**Price.** A reported finding with nobody assigned to it is a finding that stays. The count of two is pinned in a test of this feature's, so the feature that repairs `security-options.ts` breaks a test of mine and will have to read this entry to know that is correct. This entry was restated on its own branch before merge and the restatement is disclosed here rather than made quietly: it first reported the intermittent failures of `test/limit-account.test.ts` and the race files as somebody else's, on the reasoning that they looked like load rather than like anything this branch changed. They were this branch's, they are E-917's, and reporting a fault of one's own as a neighbour's is the worst way to be wrong about ownership.
