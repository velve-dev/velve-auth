...
**Context.** §5 requires a planted fault before a passing check is trusted, and E-755 records the reviewer doing that nine times. The new guard was planted three ways, with the outcome predicted first.
...
**Context.** All nine citations E-772 and E-773 name are moved to the entry whose subject they carry. Each was verified by reading the line and the entry rather than by trusting the list: the seven in `src/` and the two in the reference all pointed one entry below their subject.
...
`instance.ts` twice, which is the file the wave was cut to vacate. `migrate()` ran the core migrations only, so `plugin` had to edit it; `core/plugin/migrations.ts` now contributes and returns nothing, which leaves E-759's decision — that plugin migrations do not run until the ledger's version collision is settled — exactly where it was. `SeamSurface` was not parameterised by the identity mode while `VelveAuth<M>` is, so `email-flows` would have edited it to make `/email/*` conditional; each seam module now declares its own `…Surface<M>` and the assembly intersects the three.
