...
*Preis:* Bis dahin bleibt die Reihenfolge der Testdateien ein Faktor für das Ergebnis der Testsuite. Das ist genau die Sorte Flackern, die man später für ein Datenbankproblem hält.
...
*Preis:* Der Lauf ist an eine Umgebungsvariable gebunden (`VELVE_NIGHTLY=1`), weil ein eigenes nächtliches Vitest-Projekt `vitest.config.ts` bräuchte und ein eigener Skriptname `package.json` — beides gehört diesem Feature nicht. Bis jemand den nächtlichen Auftrag einrichtet, läuft die Prüfung nirgends automatisch. Die Verpflichtung je Commit deckt weiterhin T-RAND-4 mit seiner eigenen Schwelle von 1000 Werten ab; T-RAND-Kollision, eine Million Werte in acht Arbeitern, ist weiterhin nicht geschrieben.
