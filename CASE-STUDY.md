...
**E-160 — Eine Parameterliste braucht mindestens einen nicht leeren Wert.**
*Kontext:* Die PHC-Grammatik erlaubt einem Parameter einen leeren Wert, und das Salzfeld eines importierten `$fbscrypt$`-Strings trägt Base64-Auffüllung. Beides zusammen macht das Feld `aac=` mehrdeutig: lesbar als Parameter `aac` ohne Wert und als aufgefülltes Salz. Wer falsch rät, verschiebt Salz und Hash um ein Feld; der Verifizierer scheitert dann ohne Fehlermeldung und meldet „falsches Kennwort".
*Verworfen:* (a) Auffüllung im Salz- und im Hashfeld zu verbieten, wie die PHC-Spezifikation selbst es tut. (b) Die Felder von rechts abzuzählen.
*Grund:* (a) hätte die Übernahme von Supabase- und Firebase-Beständen gebrochen: GoTrue speichert `$fbscrypt$…$<salt_b64>$<hash_b64>` mit Auffüllung, und Abschnitt 4.4 verlangt ausdrücklich, genau diesen String unverändert zu übernehmen. (b) scheitert daran, dass das Hashfeld optional ist — von rechts gezählt ist ein Salz ohne Hash nicht von einem Hash ohne Salz zu unterscheiden. Ein Feld gilt deshalb nur dann als Parameterliste, wenn mindestens ein Paar einen nicht leeren Wert trägt. Keines der acht Verfahren aus 3.3 benutzt einen leeren Parameterwert, und die Auffüllung besteht ausschließlich aus leeren Werten.
*Preis:* Ein künftiges Verfahren, dessen einziger Parameter absichtlich leer ist, würde als Salz gelesen. Das steht in der Referenz, und die Weiche ist eine geschlossene Aufzählung — ein solches Verfahren käme nicht unbemerkt hinzu.

**E-161 — Der Base64-Codec des Kennwortmoduls ist eigener Code neben dem des Schlüsselmoduls.**
*Kontext:* `src/core/keys/base64url.ts` enthält bereits einen handgeschriebenen Decoder. Er benutzt das URL-Alphabet, während PHC das Standardalphabet mit `+` und `/` verlangt, und er ist bewusst streng: Nur die kanonische Schreibweise wird angenommen, damit ein vertippter Wurzelschlüssel auffällt statt still zu denselben Bytes zu decodieren.
...
**E-168 — Der Beschleuniger wird nur für Argon2-Version 1.3 benutzt.**
*Kontext:* 2.7 sagt zu, dass `hash-wasm` bytegleiche Ausgaben liefert und ein Wechsel keine Migration erfordert; S-DEFAULT-7 macht daraus eine Anforderung. Eine Gegenprobe über beide Versionen zeigte etwas anderes: `hash-wasm` nimmt die Option `version` entgegen und **ignoriert sie**. Für `version: 0x10` liefert es dasselbe Ergebnis wie für `0x13`, während `@noble/hashes` korrekt zwei verschiedene Werte liefert.
*Verworfen:* (a) Die Abweichung als unerheblich zu behandeln, weil die Bibliothek selbst nur 0x13 erzeugt. (b) Version 1.0 gar nicht mehr zu prüfen.
*Grund:* (a) ist genau der Fehler, den S-DEFAULT-7 verhindern soll: Ob `hash-wasm` installiert ist, entschiede darüber, ob ein importierter Argon2-1.0-Hash verifiziert — dieselbe Anmeldung ginge auf einem Server durch und auf dem anderen nicht. (b) hätte einen Bestand ausgesperrt, der nach 3.3 verifizierbar sein muss. Der Beschleuniger wird deshalb nur für 0x13 gewählt; jede andere Version läuft über den reinen Pfad. Das ist die einzige Fassung, in der Anwesenheit oder Abwesenheit der Abhängigkeit nur die Laufzeit ändert.
*Preis:* Die Auswahl der Rechenmaschine hängt jetzt von einem Feld des gespeicherten Strings ab, nicht nur von der Umgebung. Ein Leser muss wissen, warum — der Kommentar sagt es, und die Gegenprobe steht als Test daneben.

**E-169 — `$2x$` wird als `$2a$` geprüft.**
*Kontext:* Die Weiche in 3.3 führt vier bcrypt-Präfixe. `bcryptjs` kennt drei davon und wirft bei `$2x$` „Invalid salt revision". Ein importierter `$2x$`-Hash wäre damit dauerhaft nicht prüfbar und jede Anmeldung darauf ein stiller Fehlschlag.
*Verworfen:* (a) `$2x$` scheitern zu lassen und den Nutzer in den Reset-Pfad zu schicken. (b) Die crypt_blowfish-Variante nachzubauen.
*Grund:* `$2x$` und `$2a$` unterscheiden sich ausschließlich darin, wie crypt_blowfish Bytes mit gesetztem höchsten Bit behandelt hat. Für ein ASCII-Kennwort sind beide Ableitungen identisch, für jedes andere können sie nicht zufällig übereinstimmen — die Umschreibung kann eine sichere Fehlanmeldung in eine richtige Antwort verwandeln und niemals eine falsche Annahme erzeugen. (a) hätte einen Bestand verworfen, der zum größten Teil aus ASCII-Kennwörtern besteht. (b) hieße, eine bekannte Fehlimplementierung in den Kern zu holen, um einen Randfall zu bedienen; das ist der falsche Ort für diesen Aufwand.
*Preis:* Ein `$2x$`-Kennwort mit Nicht-ASCII-Zeichen scheitert weiterhin und führt in den Reset-Pfad. Die Bibliothek kann nicht sagen, ob sie es mit dem einen oder dem anderen Fall zu tun hat, und die Antwort ist in beiden Fällen dieselbe — was sie sein muss.

**E-170 — Der Bezeichner der optionalen Abhängigkeit wird zusammengesetzt, nicht geschrieben.**
*Kontext:* `hash-wasm` ist eine optionale Peer-Abhängigkeit. Als Zeichenkettenliteral in `import()` erzeugt sie zwei Befunde: Die Dead-Code-Prüfung des Hauptors meldet „Referenced optional peerDependencies" und schlägt fehl, und ein Bündler eines Aufrufers versucht das Paket zur Bauzeit aufzulösen, das fehlen darf.
*Verworfen:* (a) `hash-wasm` in `knip.json` unter `ignoreDependencies` einzutragen. (b) Den Beschleuniger nicht selbst zu suchen, sondern über die Konfiguration hereinreichen zu lassen.
...
*Grund:* Ein selbst erzeugter Vektor beweist nur, dass der Verifizierer zu seiner eigenen Ableitung passt. Genau diesen Fehler beschreibt Fallstrick 1: Ableitung und Prüfung wären beide vertauscht, der Test grün, und der erste echte Firebase-Bestand scheiterte still. Der Vektor stammt aus der öffentlichen Referenzimplementierung, gehört zu einem erfundenen `user1` und ist kein Zugangsdatum irgendeines Kontos.
*Preis:* Eine Zeichenkette im Repository, die formal ein Kennwort und ein Hash dazu ist. Sie ist als solche gekennzeichnet und mit ihrer Quelle versehen; wer die Regel wörtlich liest, sieht eine Ausnahme, und sie steht hier, damit sie beim Durchsehen nicht übersehen wird.

**E-172 — Der Markentyp `Secret<…>` entsteht im Kennwortmodul.**
*Kontext:* S-TIM-3 verbietet `===`, `startsWith`, `includes` und `localeCompare` auf einem Wert vom Typ `Secret<…>`, und T-TIM-3 prüft das statisch. Die Spezifikation definiert den Typ nirgends, und keine der bisherigen Wellen hat ihn eingeführt — die Prüfung hätte also nichts gefunden und wäre grün gewesen, weil es keine markierten Werte gab.
*Verworfen:* Den Typ dort zu lassen, wo er hingehört, nämlich in einem gemeinsamen Modul, und ihn zusammen mit dem Modul zu bauen, das ihn als zweites braucht.
*Grund:* Eine statische Prüfung, die nichts findet, weil sie nichts findet, ist genau der Fall, den Regeln §5 als „grün, weil auf keine Datei zutreffend" benennt. Der Typ entsteht deshalb dort, wo der erste markierte Wert entsteht: bei der Ausgabe des KDF. Ein gemeinsamer Ort wäre eine Datei außerhalb der Zuständigkeit dieses Bausteins.
*Preis:* Der Typ liegt im falschen Modul. Sobald das Sitzungs- oder das Faktormodul ihn braucht, muss er umziehen, und der Umzug betrifft eine Datei, die dann zwei Bausteinen gehört. Das ist gemeldet.

**E-173 — Ein fehlendes `v=`-Feld bedeutet Argon2-Version 1.0.**
*Kontext:* Der PHC-String eines importierten Argon2-Hashes muss das Versionsfeld nicht tragen. Was dann gilt, entscheidet, ob der Hash verifiziert.
*Verworfen:* Ein fehlendes Feld als 1.3 zu lesen, weil das die einzige Version ist, die die Bibliothek selbst erzeugt.
*Grund:* Der Referenzdecoder von Argon2 (`argon2_decode_string`) setzt bei fehlendem `v=` die Version 1.0, und jedes Werkzeug, das solche Strings erzeugt hat, hat sie unter dieser Annahme erzeugt. 1.3 anzunehmen wäre bequem und würde jeden dieser Hashes falsch ableiten — wieder ohne Fehlermeldung, wieder als „falsches Kennwort".
*Preis:* Die Bibliothek verifiziert damit eine Version, die sie nie erzeugt, und der Beschleuniger kann sie nicht rechnen (E-168). Beides zusammen heißt: Ein Bestand ohne Versionsfeld läuft langsamer als einer mit. Nach dem ersten Login ist er ohnehin auf Argon2id 1.3 gehoben.

**E-174 — Der Weg ohne Nutzer fragt trotzdem nach einer Anmeldedatei, und zwar nach der Null-UUID.**
*Kontext:* S-TIM-1 verlangt für existierende und nicht existierende Kennungen dieselbe Folge von Datenbank- und KDF-Aufrufen, und T-TIM-1b prüft vier Fälle auf byteweise identische Aufrufsequenzen. Die Auflösung des Nutzers gehört einem anderen Baustein; dieser bekommt eine Nutzer-ID oder `null`.
*Verworfen:* Bei `null` die Abfrage der Anmeldedatei zu überspringen und nur den KDF-Aufruf gegen den Dummy zu machen.
*Grund:* Dann wäre die Folge um genau eine Abfrage kürzer — messbar, und zwar in einem Bereich (0,1–2 ms nach 5.1 (a)), der über das Netz mit ein paar hundert Messungen sichtbar wird. Der Dummy-KDF-Aufruf hätte den größeren Unterschied verdeckt und den kleineren stehen lassen. Abgefragt wird deshalb immer, und wenn keine Nutzer-ID vorliegt, mit der Null-UUID: eine syntaktisch gültige `uuid`, die `gen_random_uuid()` nicht erzeugt und die auf denselben Primärschlüsselindex trifft wie jede andere.
*Preis:* Eine Datenbankabfrage je Anmeldeversuch auf eine nicht existierende Kennung, die nie eine Zeile liefert. Das ist ein Indexzugriff ohne Treffer, und es ist genau der Zugriff, den der Echtfall auch macht. Wer die Zeile mit der Null-UUID anlegt, hebt die Gleichförmigkeit auf — sie kann nicht angelegt werden, weil `velve.user` keine solche Zeile hat und der Fremdschlüssel es verbietet.

**E-175 — Ein unlesbarer Schlüsselstand wird nicht als falsches Kennwort verkleidet.**
*Kontext:* Nach L-2 liegt der PHC-String verschlüsselt. Fehlt die genannte Schlüsselversion im Ring, wirft das Schlüsselmodul `key_version_unknown` (S-KEY-4). Der Prüfpfad müsste das nach L-1 eigentlich in dieselbe einheitliche Fehlantwort überführen wie jeden anderen Misserfolg.
*Verworfen:* Die Ausnahme zu fangen und die Anmeldung als `invalid_credentials` abzulehnen.
...
*Grund:* Die frühe Ablehnung wäre um einen ganzen KDF-Aufruf schneller als jede andere Antwort und damit ein Orakel mit einem Signal von 50 bis 250 ms — nicht dafür, ob ein Konto existiert, sondern dafür, ob es aus dem Import stammt. Das ist eine Liste, die ein Angreifer haben will: Konten mit einem alten Hash sind die mit dem alten, oft wiederverwendeten Kennwort. Also läuft auch dieser Fall über den Dummy, mit denselben Parametern, über denselben Semaphor.
*Preis:* Ein Betreiber, der `acceptLegacy` leert, bezahlt für jede Anmeldung auf einer stillgelegten Anmeldedatei denselben Speicher und dieselbe Zeit wie für eine echte. Das ist der Preis dafür, dass die Stilllegung von außen nicht sichtbar ist, und der richtige Weg aus dem Zustand ist ohnehin der Reset-Pfad, nicht das Aushungern des Prüfpfads.

**E-177 — Die Spalte und die Anmeldedatei müssen dieselbe Funktion nennen.**
*Kontext:* `acceptLegacy` wird auf der Klartextspalte `scheme` ausgewertet — sie ist nach L-2 die einzige Angabe, die ohne Schlüssel lesbar ist und den Bestand auswertbar macht. Der Argon2-Verifizierer nahm seine Variante dagegen aus dem Bezeichner **innerhalb** der entschlüsselten Anmeldedatei. Der Prüfer hat es durchgespielt: Eine Zeile mit `scheme='argon2id'`, deren Inhalt `$argon2i$…` ist, wird bei `acceptLegacy: []` mit `verified` beantwortet. Ein Betreiber, der jedes Altverfahren abgeschaltet hat, prüft weiterhin Argon2i und Argon2d.
*Verworfen:* (a) Die Politik auf dem entschlüsselten Bezeichner auszuwerten statt auf der Spalte. (b) Die Spalte beim Lesen gegen den Inhalt zu korrigieren.
*Grund:* (a) hätte die Auswertbarkeit des Bestands aufgegeben, die L-2 ausdrücklich kauft: Wer wissen will, wie viele bcrypt-Zeilen noch stehen, dürfte dann nicht mehr `SELECT scheme, count(*)` rechnen, sondern müsste jede Zeile entschlüsseln. (b) hätte einen stillen Schreibvorgang auf dem Prüfpfad erzeugt und die Uneinigkeit verdeckt, statt sie zu beantworten. Stattdessen ist die Übereinstimmung jetzt eine Vorbedingung: Der PHC-Bezeichner muss die Spalte wörtlich wiederholen, sonst antwortet die Weiche `false`. Die Prüfung liegt in `overPhc` und gilt damit für alle sieben PHC-Verfahren auf einmal, nicht nur für Argon2.
*Preis:* Eine Zeile, deren Spalte durch einen fehlerhaften Import nicht zum Inhalt passt, ist ab sofort nicht mehr prüfbar — vorher hätte sie funktioniert. Das ist beabsichtigt und führt in den Reset-Pfad: Eine Anmeldedatei, über die die Datenbank etwas anderes behauptet als sie selbst, ist keine, auf die man eine Sitzung ausstellt.

**E-178 — Kein Nachschlagen im Modul liest einen Namen von `Object.prototype`.**
*Kontext:* Die Weiche war ein Objektliteral, indiziert mit der Spalte `scheme`; die PBKDF2-Digest-Tabelle ebenso, indiziert mit dem Bezeichner der geparsten Anmeldedatei. `constructor` löst auf beiden zu einer Funktion auf, und `await Object(password, stored)` liefert ein Objekt, das der Aufrufer als Treffer liest. Der zweite Fall ist der erreichbare: Der Bezeichner kommt aus dem gespeicherten String, `constructor` besteht die Bezeichner-Regex, und nur ein späterer Fehler im Ableitungspfad machte daraus `false`.
*Verworfen:* (a) Den Rückgabewert am Ende auf `=== true` zu prüfen. (b) `Object.create(null)` als Prototyp der Tabellen.
*Grund:* (a) hätte den Symptomwert abgefangen und den Aufruf einer beliebigen geerbten Funktion mit dem Kennwort als erstem Argument stehen gelassen. (b) wäre korrekt, aber der Typ sagte weiterhin „Objekt mit beliebigen Schlüsseln", während `Map` schon in der Signatur festhält, welche Schlüssel es gibt, und für jeden anderen `undefined` liefert. Dasselbe gilt für die Prüfung, ob der Beschleuniger die drei Funktionen hat: Sie fragt jetzt `Object.hasOwn`, bevor sie liest.
*Preis:* Drei Nachschlagetabellen sind länger zu schreiben als ein Literal, und `Map` erlaubt keine `as const`-Ableitung des Schlüsseltyps — er steht jetzt zweimal da, im Typargument und in den Einträgen. Der Prüfer hat den Fall gefunden, nicht der Autor; das steht hier, weil es zur Beurteilung gehört, wie belastbar die übrigen Nachschlagestellen sind.

**E-179 — Ein toter Schlüsselstand wird beim Start gemeldet, nicht bei der Anmeldung. E-175 ist damit aufgehoben.**
*Kontext:* E-175 hat die Ausnahme aus dem Schlüsselmodul bewusst nach oben durchgereicht, damit ein entfernter Schlüsselstand nicht als „falsches Kennwort" verschwindet. Der Prüfer hat zwei Dinge dagegengehalten. Erstens verletzt der `throw` S-TIM-1 wörtlich: zwischen Schritt 2 und Schritt 4 gibt es weder `return` noch `throw`, ohne Vorbehalt. Zweitens — und das benennt E-175 nicht — trennt genau dieser Zweig die Nutzer in „vor der Rotation geschrieben" und „danach geschrieben" und ist damit während jedes Rotationsfensters ein Aufzählungskanal, nicht nur nach einem Betriebsfehler.
*Verworfen:* (a) Der `throw` bleibt (E-175). (b) Der Fehlschlag wird als `invalid_credentials` verkleidet und sonst nichts.
*Grund:* Die Wahl zwischen (a) und (b) war falsch gestellt; beide Antworten liegen auf dem Anfragepfad, und dort gehört die Meldung nicht hin. Sie richtet sich an den Betreiber, nicht an den Nutzer, und sie ist einmal richtig statt bei jeder Anmeldung. `assertStoredKeyVersionsAreKnown` liest die vorhandenen `key_version`-Werte aus `password_credential` und hält sie gegen den Ring — eine Abfrage, beim Zusammenbau, laut und mit den fehlenden Ständen im Fehler. Der Anmeldepfad fängt den Fehlschlag danach still ab und prüft gegen den offen gehaltenen Dummy-String weiter, also mit **einem** Entschlüsselungsversuch und **einem** Verifizierer-Aufruf wie jeder andere Fall auch.
*Preis:* Der Dummy hält seinen PHC-String zusätzlich im Klartext im Speicher. Das ist der Hash eines zufällig gezogenen Wertes und niemandes Kennwort, aber es ist eine zweite Darstellung desselben Wertes, und wer den Speicher liest, sieht sie. Der zweite Preis: Die Startprüfung gehört in den Zusammenbau, und der gehört diesem Baustein nicht — sie wird exportiert und muss aufgerufen werden. Bis das geschieht, ist der Betriebsfehler still.

**E-180 — Der Bezeichner des Beschleunigers steht wieder als Literal da. E-170 ist damit erledigt.**
*Kontext:* E-170 hat den Bezeichner zusammengesetzt, weil `knip.json` einem Baustein dieser Welle nicht gehört und die Dead-Code-Prüfung eine referenzierte optionale Peer-Abhängigkeit als Befund meldet. Der Eintrag hat ausdrücklich darum gebeten, das rückgängig zu machen; `main` trägt `hash-wasm` jetzt in `ignoreDependencies`.
*Verworfen:* Die Zusammensetzung zu behalten, weil sie einen Bündler auch daran hindert, ein absichtlich fehlendes Paket aufzulösen.
*Grund:* Der zweite Grund aus E-170 trägt nicht so weit, wie er dort formuliert war, und der Preis war größer als dort benannt: Nicht nur die Dead-Code-Prüfung sieht die Kante nicht, sondern auch jeder Abhängigkeits- und Sicherheitsscanner. Eine Meldung gegen `hash-wasm` wäre nie als „betrifft diese Zeile" aufgetaucht, und der Prüfer konnte den Beschleuniger nicht unter seinem echten Namen instrumentieren, ohne das Tor zu brechen. Ein dynamischer Import ist ohnehin kein statischer; ein Bündler, der ihn nicht auflösen kann, warnt, statt zu scheitern.
*Preis:* Die Ausnahme steht jetzt in einer Datei, die kein Feature besitzt, und muss dort stehen bleiben. Wer `hash-wasm` je aus `package.json` entfernt, muss auch `knip.json` anfassen, sonst meldet die Prüfung eine Ausnahme für ein Paket, das es nicht gibt.

**E-181 — Die billige Vorprüfung ist eine Schranke, keine Messung. E-164 Schritt 1 war falsch.**
*Kontext:* E-164 begründet die erste der drei Messungen damit, dass eine UTF-8-Kodierung nie kürzer ist als die Zahl der UTF-16-Code-Einheiten. Das stimmt — für dieselbe Zeichenkette. Die Prüfung vergleicht aber die Code-Einheiten der **rohen** Eingabe gegen die Byte-Obergrenze der **normalisierten** Form, und NFKC kann zusammensetzen: `U 0308 0301` sind drei Code-Einheiten und fünf Bytes und werden zu einer Code-Einheit und zwei Bytes. `"Ǘ"` in zerlegter Schreibweise, 1400-mal wiederholt, sind 4200 Code-Einheiten und normalisieren zu 2800 Byte — die Politik lässt es zu, die Vorprüfung wies es ab. Kein Loch, sondern eine Aussperrung, und zwar genau für die Eingaben, für die die Normalisierung gedacht ist.
*Verworfen:* (a) Die Vorprüfung zu streichen und immer zuerst zu normalisieren. (b) Zuerst die rohe UTF-8-Länge zu messen und diese zu vergleichen.
*Grund:* (a) hätte den zweiten Angriffsvektor aus 5.18 (a) wieder geöffnet: Ein „Kennwort" von einem Megabyte würde normalisiert, bevor irgendeine Grenze greift. (b) hat denselben Fehler wie vorher, nur in der richtigen Einheit — auch die rohe Byte-Länge ist nicht die der Normalform. Es gibt keine billige exakte Messung; es gibt nur eine Schranke. UAX #15 begrenzt die Schrumpfung durch kanonische Komposition in UTF-8 auf den Faktor drei, die Vorprüfung rechnet mit vier. Damit weist sie nichts ab, was bestehen würde, und sie hält weiterhin alles fern, was groß genug ist, um zu schaden.
*Preis:* Die Bibliothek normalisiert im Grenzfall bis zu 16 KiB statt 4 KiB, bevor sie ablehnt. Und die Zahl vier ist eine Sicherheitsmarge auf eine Zusicherung des Unicode-Standards, nicht auf etwas, das dieser Code selbst nachrechnet — ändert Unicode diese Zusicherung, merkt es hier niemand.

**E-182 — Ein gespeicherter Kostenparameter hat eine Obergrenze, und sie ist nicht einstellbar.**
*Kontext:* S-DOS-3 sagt zu, dass der belegte Speicher das Produkt aus Semaphorgröße und Speicherparameter nicht überschreitet. Der Speicherparameter steht aber je Datensatz im gespeicherten String, und wer ihn schreibt, ist der Import. `m`, `ln` und `i` durften bis zu zehn Stellen haben und liefen ungeprüft ins KDF; `needsRehash` vergleicht nur nach **unten**. Die tatsächliche Schranke war damit der größte Wert, den je ein Import geschrieben hat.
*Verworfen:* (a) Die Obergrenze aus den konfigurierten Argon2id-Parametern abzuleiten. (b) Sie als weitere Konfigurationsoption zu führen.
*Grund:* (a) hätte jeden importierten Hash abgewiesen, der stärker ist als die eigene Politik — genau die Bestände, die man am wenigsten zum Zurücksetzen zwingen will. (b) wäre ein Schalter, dessen einzige Bewegungsrichtung nach oben eine Abschwächung ist; nach S-DEFAULT-1 müsste er ausdrücklich gesetzt und beim Start protokolliert werden, und dafür ist der Fall zu selten. Die Grenzen stehen fest, mit Abstand über allem, was die fünf Quellen aus Abschnitt 4 tatsächlich liefern: 64 MiB Speicher gegen 32 MiB bei Better Auths scrypt und 16 MiB bei Firebase, zwei Millionen PBKDF2-Runden gegen Djangos 1,2 Millionen.
*Preis:* Ein Bestand, der die Grenze überschreitet, ist nicht prüfbar und führt in den Reset-Pfad — ohne Konfigurationsmöglichkeit, also ohne Ausweg für den Betreiber außer einem Fehlerbericht. Das ist bewusst: Eine Zahl, die nur geraten und nie gemessen wurde, gehört nicht in eine Option, sondern in eine Meldung mit Daten daneben.

**E-183 — Ohne gemeldete Kernzahl ist die Antwort eins. E-162 ist damit aufgehoben.**
*Kontext:* E-162 hat bei unbekannter Kernzahl auf die Obergrenze 4 zurückgefallen und den Durchsatz als Grund genannt. Der Prüfer hat die Anforderung dagegengehalten: S-DOS-3 sagt `min(4, cpus)`, und T-DOS-3 misst genau das. Auf einem Ein- oder Zweikern-Container ist 4 nicht die Obergrenze, sondern deren Überschreitung — die Vorgabe verfehlt die Anforderung, um die es geht, und zwar auf der Sorte Maschine, auf der der Speicher am knappsten ist.
*Verworfen:* (a) Bei 4 zu bleiben (E-162). (b) `node:os.availableParallelism()` zu benutzen, das Node 20.19 hat. (c) `process.availableParallelism` zu lesen — das gibt es nicht, die Funktion sitzt auf `node:os`.
*Grund:* (a) verfehlt die Anforderung. (b) hieße, einen `node:`-Import in den Kern zu holen, der nach Abschnitt 2.6 Web-Standards annimmt; er müsste dynamisch geladen werden, und `resolvePasswordConfig` antwortet synchron. (c) war schlicht falsch und ist beim Übersetzen aufgefallen — hier festgehalten, weil der erste Versuch dieser Korrektur genau daran scheiterte. Bleibt `navigator.hardwareConcurrency`, und wo es das nicht gibt, die vorsichtige Zahl.
*Preis:* Auf Node 20.19 — der unteren Baugrenze — läuft ohne ausdrückliche Angabe genau eine Kennwortprüfung gleichzeitig, und vier gleichzeitige Anmeldungen stehen in einer Reihe von je 90 ms. Das ist der Durchsatzabbruch, den E-162 vermeiden wollte. Er steht jetzt in der Referenz mit der Anweisung daneben: Wer auf Node 20 fährt, setzt `concurrentHashLimit`.

**E-184 — Die Obergrenze der Eingabelänge hat drei Fehler, also drei Codes.**
*Kontext:* `maximum_length_above_ceiling` wurde für drei verschiedene Fälle geworfen — über 4096, unter `minimumLength`, und keine ganze Zahl —, und die Meldung nannte nur den ersten. Wer den zweiten Fall auslöste, bekam einen Text, der von seiner Konfiguration nichts sagte.
*Verworfen:* Die Meldung so umzuformulieren, dass sie alle drei Fälle abdeckt.
*Grund:* Ein Fehlercode ist eine maschinenlesbare Angabe (Regeln §3), und drei Ursachen unter einem Code zusammenzufassen macht ihn für den Aufrufer wertlos — er kann nicht entscheiden, welches Feld er anfassen muss. Ein Sammeltext hätte dasselbe Problem in Prosa gehabt.
*Preis:* Zwei zusätzliche Codes in einer Aufzählung, die ein Aufrufer beim Aktualisieren erschöpfend behandeln muss.

**E-185 — Das Einfügen-oder-Aktualisieren sagt die Eigentümerbedingung hin, statt sie aus dem Primärschlüssel folgen zu lassen.**
*Kontext:* Nach der Zusammenführung von `main` liest die statische Prüfung zu S-OWNER-2 jede Anweisung, die irgendwo `UPDATE` enthält — auch als `ON CONFLICT … DO UPDATE` innerhalb eines `INSERT`, weil ein datenverändernder CTE mit `WITH` beginnt und die Verankerung deshalb gefallen ist. Das Schreiben der Anmeldedatei hatte keine `WHERE`-Klausel: Der Konfliktschlüssel `user_id` ist zugleich der Primärschlüssel und die Eigentümerspalte, die Bindung war also vorhanden, aber nur, wenn man das Schema kennt.
*Verworfen:* (a) Das Tor zu bitten, `ON CONFLICT (user_id) DO UPDATE` als Eigentümerbindung anzuerkennen. (b) Die Anweisung in ein `UPDATE` mit vorangehendem `INSERT … ON CONFLICT DO NOTHING` zu zerlegen.
*Grund:* (a) hätte die Prüfung um einen Sonderfall erweitert, dessen Richtigkeit davon abhängt, dass der Konfliktschlüssel immer die Eigentümerspalte ist — eine Annahme, die für die nächste Tabelle schon falsch sein kann. (b) hätte aus einer Anweisung zwei gemacht und damit ein Zeitfenster zwischen ihnen geöffnet. Stattdessen trägt das `DO UPDATE` jetzt `WHERE credential.user_id = $1`. Die Bedingung ist redundant zum Konfliktschlüssel und deshalb kostenlos, und sie macht aus einer Eigenschaft des Schemas eine Aussage der Anweisung. Gegen PostgreSQL 16 nachgerechnet, weil `ON CONFLICT` die Zieltabelle über den Alias anspricht und nicht über den qualifizierten Namen.
*Preis:* Eine Bedingung, die nie falsch sein kann, steht in der Anweisung und muss beim Lesen als redundant erkannt werden. Der Kommentar sagt, warum sie dort steht.
*Nachtrag:* Zwei Dinge fehlten dem Eintrag. Erstens die dritte Möglichkeit, die das Haupttor gefunden hat und die diesem Eintrag vorzuziehen gewesen wäre: eine Markierung im Statement nach E-142, mit der eine Anweisung die fehlende Eigentümerbedingung selbst begründet. Sie wurde hier nicht gewählt, weil die Bedingung tatsächlich formulierbar ist und eine formulierte Bedingung mehr wert ist als eine begründete Auslassung — aber sie war die naheliegende Alternative und stand nicht im Eintrag. Zweitens, und schwerer: `ON CONFLICT … DO UPDATE … WHERE` **wirft nicht**, wenn seine Bedingung falsch ist, sondern ändert stillschweigend nichts, und `write()` warf sein Ergebnis weg. An dem Tag, an dem diese Tautologie keine mehr ist — ein `BEFORE INSERT`-Trigger, der `NEW.user_id` umschreibt, ein zusammengesetzter Konfliktschlüssel, oder dieses Muster auf eine andere Tabelle kopiert —, hätte `setPassword` Erfolg gemeldet und das Kennwort nicht gespeichert. Die Anweisung trägt jetzt `RETURNING user_id`, und eine Zeilenzahl ungleich eins ist ein Fehler; `replaceIfUnchanged` zwei Funktionen tiefer machte das von Anfang an richtig. Zwei Testvorrichtungen dieses Bausteins antworteten auf das Einfügen mit einer leeren Liste — sie behaupteten also genau den Fehlerfall und kamen damit durch; auch das hat erst die Zeilenzahl gezeigt.

**E-186 — Der Beschleuniger gibt je Ableitung einmal an die Zeitgeberphase ab.**
*Kontext:* Der Kommentar an der Tick-Konstante behauptete, das Abgeben alle 10 ms halte eine Ableitung davon ab, jede andere Anfrage zu blockieren. Für den reinen Pfad stimmt das; für den Standardfall mit installiertem `hash-wasm` nicht. Der Beschleuniger rechnet in **einem** synchronen WebAssembly-Aufruf und löst sein Versprechen in einer Mikrotask auf — die Kette aus Semaphor freigeben, nächsten Warter zulassen, wieder ableiten läuft vollständig im Mikrotask-Abarbeiten und erreicht die Zeitgeberphase nie. Das Tor hat gemessen: 800 gleichzeitige Anmeldungen bei einer Wartegrenze von 5000 ms, **14 684 ms Gesamtdauer, null Absagen**, und in dieser Zeit feuerte **kein einziger Zeitgeber im Prozess** — kein Ratenfenster, keine HTTP-Zeitüberschreitung, keine Bereitschaftsprüfung. Mit weggemocktem `hash-wasm` derselbe Ansturm: 1 geprüft, 19 `rate_limited`.
*Verworfen:* (a) Zu dokumentieren, dass S-DOS-4 mit `hash-wasm` nicht gilt. (b) Den Beschleuniger in Blöcke zu zerlegen, wie `asyncTick` es beim reinen Pfad tut. (c) `scheduler.yield()` zu benutzen, wo es das gibt.
*Grund:* (a) hätte eine Anforderung in der Vorgabestellung aufgegeben — die Abhängigkeit ist optional, aber sie ist installiert, sobald jemand sie einträgt, und dann verhält sich die Bibliothek anders, als sie zusagt. Genau das verbietet S-DEFAULT-7. (b) ist nicht möglich: Der Aufruf ist eine einzige WASM-Funktion ohne Einstiegspunkt in der Mitte. Was geht, ist das Abgeben **zwischen** den Ableitungen, und das genügt: Der 20-ms-Block eines einzelnen Aufrufs ist kürzer als die 10 ms × mehrere Runden des reinen Pfads, und die Zeitgeber laufen dazwischen. (c) wäre die modernere Schreibweise und die falsche: `scheduler.yield()` kehrt in eine Fortsetzungswarteschlange zurück, und was hier laufen muss, ist die Zeitgeberphase. `setTimeout(…, 0)` sagt genau das.
*Preis:* Eine Zeitgeberrunde je Ableitung, also etwa eine Millisekunde auf zwanzig — rund fünf Prozent des Vorteils, den der Beschleuniger bringt. Und die Bibliothek gibt jetzt an zwei verschiedenen Stellen unterschiedlich ab, je nach Rechenmaschine; wer die eine ändert, muss an die andere denken. Der Prüfplan hält das fest: Der Testfall läuft gegen die Maschine, die die Laufzeit tatsächlich wählt, und schlägt fehl, wenn sie die Zeitgeberphase aushungert.

**E-187 — Die Spalte wird schon beim Schreiben gegen die Anmeldedatei gehalten.**
*Kontext:* E-177 hat die Übereinstimmung von `scheme`-Spalte und PHC-Bezeichner zur Vorbedingung des Prüfens gemacht. Auf der Schreibseite gab es sie nicht: Der Aufrufer — der Kern beim Registrieren, der Importeur beim Schreiben eines Bestands — konnte jede Kombination ablegen. Eine Zeile mit widersprüchlicher Spalte ist nach E-177 dauerhaft nicht prüfbar, und der Nutzer erfährt das als „falsches Kennwort", ohne dass irgendwo ein Fehler steht.
*Verworfen:* (a) Den Parameter `scheme` zu streichen und ihn im Repository aus dem String abzuleiten. (b) Es beim Lesen zu belassen und den Importeur in der Dokumentation zu warnen.
*Grund:* (a) wäre die dichteste Fassung und hätte die Widersprüchlichkeit unmöglich gemacht statt nur verboten; verworfen, weil acht Aufrufstellen in den Tests dieses Bausteins die Spalte ausdrücklich mitgeben und ihre Angabe genau das ist, was der Test prüfen will — ein abgeleiteter Wert könnte nicht mehr falsch sein und damit auch nicht mehr geprüft werden. (b) verschiebt einen Fehler, den die Bibliothek in einer Zeile findet, auf den Betrieb eines fremden Bestands. Also prüft das Repository die Übereinstimmung, bevor es verschlüsselt, und wirft `scheme_does_not_match_credential`. Das ist zugleich der erste Aufrufer von `schemeOfStoredHash` in `src/` — die Funktion, die die Weiche aus 3.3 trägt, hatte bis dahin nur Tests.
*Preis:* Der Importeur kann eine Zeile nicht mehr „so wie sie kam" ablegen, wenn Quellspalte und Quellstring uneins sind; er muss sich entscheiden, und die Entscheidung ist seine. Für den Fall, dass eine Quelle beides liefert und beides widerspricht, gibt es damit keinen stillen Weg mehr — nur einen Fehler beim Import oder einen Datensatz weniger.

**E-188 — `concurrentHashLimit` senkt die Grenze und hebt sie nie.**
*Kontext:* Die Referenz sagte zu, jede Grenze dieses Moduls zeige in die sichere Richtung und ein Abschwächungsversuch werde abgewiesen. Für den Semaphor stimmte das nicht: `resolvePasswordConfig({ concurrentHashLimit: 100_000 })` wurde angenommen und behauptete damit eine Speicherschranke von 1855 GiB — auf genau dem S-DOS-3-Argument, auf dem der Absatz ruht. Dazu kam bcrypt: Die vier Kostengrenzen aus E-182 galten für „jede gespeicherte Anmeldedatei", nur hat bcrypt keinen Speicherparameter und war gar nicht dabei. Eine importierte `$2a$31$`-Zeile belegt einen Semaphorplatz für Stunden.
*Verworfen:* (a) Die Zusage in der Referenz abzuschwächen, statt den Code zu binden. (b) Die Obergrenze für `concurrentHashLimit` aus der gemeldeten Kernzahl abzuleiten, damit eine Maschine mit 64 Kernen mehr bekommt.
*Grund:* (a) hätte die Aussage gerettet und die Anforderung aufgegeben. S-DOS-3 nennt `min(4, cpus)` als **die** Schranke der Bibliothek, nicht als Startwert, und T-DOS-3 misst genau das — eine Installation mit einem höheren Wert erfüllt die Anforderung nicht, gleich wie groß die Maschine ist. (b) hätte dasselbe Problem in eine Formel verlegt. Die Option senkt jetzt und hebt nie; wer mehr gleichzeitige Ableitungen braucht, fährt mehr Prozesse. Für bcrypt ist die Kostenzahl der einzige verfügbare Riegel, und 14 liegt vier Stufen über dem, was GoTrue, Auth0 und Clerk schreiben.
*Preis:* Zwei Werte, die eine Konfiguration bisher annahm, sind jetzt Startfehler, und ein Bestand mit bcrypt-Kosten über 14 ist nicht mehr prüfbar und geht in den Reset-Pfad. Der zweite Preis ist ehrlicher zu benennen: Beide Lücken standen in der Referenz als Zusage, bevor sie im Code standen — die Dokumentation lief dem Code voraus, und das ist die Reihenfolge, in der eine Zusage falsch wird.
