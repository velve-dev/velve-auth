...
describe("the statements this repository actually runs (S-FIX-2, E-23)", () => {
	...
	it("gives every row-changing statement an owner predicate or the token hash", async () => {
		const changing = (await statementsAsTheyRun()).filter((sql) =>
			/^(UPDATE|DELETE)/.test(sql.trimStart()),
		);
		const unscoped = changing.filter(
			(sql) => !/\bWHERE\b[\s\S]*\b(user_id|token_sha256)\s*=/i.test(sql),
		);
