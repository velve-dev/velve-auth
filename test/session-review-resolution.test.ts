...
describe("L-4, S-CACHE-3: account_disabled and where it may appear", () => {
	...
	it("is raised in exactly one place in the library, and that place is session resolution", () => {
		...
		// The assembly names the code without raising it, in the route contract D.3 fixes for every
		// route with caller `session`. It named it a second time until `instance.ts` stopped keeping
		// its own copy of the twenty-five codes and read the error map's own list (E-734); the fourth
		// file is `oauth/routes.ts`, whose three `identity` rows have that caller (E-564).
