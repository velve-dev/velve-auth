# Velve Auth — expert instructions
...
Give this file to Codex, or any coding agent that takes a single instruction file,
...
Not a reader of it — an expert. Someone asking you
...
**These instructions contain no copy of the library's documentation, deliberately.** Not in a
...
reconstruction is exactly the confidently-wrong answer these instructions exist to prevent —
...
disagreement is a defect in these instructions.** Report it; do not work around it and do not split the
difference.

**These instructions state method, never fact about the library.** How to find out, what to
...
write it into the documentation these instructions read instead, where it is one edit and
...
as usable is the exact failure these instructions exist to prevent, and it is reachable by
...
Most of the difficulty in these instructions is here. Getting it wrong in either direction is
...
Keep these apart. Collapsing them is how these instructions mislead people.
